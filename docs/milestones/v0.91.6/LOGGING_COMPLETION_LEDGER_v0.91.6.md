...
| `#4001` | GitHub, token, release, and projection observability | `complete` | [`GITHUB_TOKEN_RELEASE_PROJECTION_PROOF_4001.md`](review/logging_observability/GITHUB_TOKEN_RELEASE_PROJECTION_PROOF_4001.md); [`WP03_TOOLING_PROOF_LOOP_CLOSEOUT_4048.md`](review/logging_observability/WP03_TOOLING_PROOF_LOOP_CLOSEOUT_4048.md); `adl/src/cli/pr_cmd/github_client.rs`; `adl/src/cli/tooling_cmd/github_release.rs` | Completes the shared token-policy reuse, native release publish proof, and bounded projection-convergence status for WP-03. |
...
| `#3963` deterministic token-loading substrate | partially_consumed | `#4001` reuses the shared ADL GitHub client policy for release tooling; the broader resolver/cache work later closed independently through merged PR `#4042`, so this row remains partial rather than silently broadened into full consumption. |
| `#3965` draft-release publish gap | consumed | Native octocrab-backed draft and publish proof is part of the `#4001` packet, and the issue itself is now closed so this consumed label no longer contradicts live state. |
| `#3985` existing-issue metadata repair gap | routed | Explicitly preserved as adjacent tooling reliability work, not required to complete WP-03 logging/release observability; the routed follow-on later completed independently through merged PR `#4117`. |
...
- Existing-issue metadata repair for title/label/body parity remained outside
  this bounded WP-03 slice rather than being silently consumed by it; that
  adjacent follow-on later closed independently through `#3985` / PR `#4117`.

## Closeout Rule

WP-03 may close only when this ledger remains truthful, the linked proof packet
and closeout note exist, and no row above is silently treated as “future work.”
