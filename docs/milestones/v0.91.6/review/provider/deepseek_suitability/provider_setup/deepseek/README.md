# Provider setup: deepseek

This bundle gives you a local starting point for configuring the `deepseek` provider family.
...
Steps:
1. Copy `env.example` to a local untracked env file and put your real credential in `DEEPSEEK_API_KEY`.
...
Notes:
Use this for the Rust-native DeepSeek provider path. The default endpoint is DeepSeek's chat completions API; override config.endpoint only for tests or a trusted compatible endpoint.
