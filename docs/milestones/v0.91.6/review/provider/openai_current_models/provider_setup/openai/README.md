...
This bundle gives you a local starting point for configuring the `openai` provider family.
...
Steps:
1. Copy `env.example` to a local untracked env file and put your real credential in `OPENAI_API_KEY`.
...
Notes:
Use this for the Rust-native OpenAI provider path. The default endpoint is OpenAI's Responses API; override config.endpoint only for tests or a trusted compatible endpoint.
