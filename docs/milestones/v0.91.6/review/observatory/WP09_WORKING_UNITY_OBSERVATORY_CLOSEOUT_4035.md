...
and umbrella `#3974` completed its closeout-truth publication issue.
...
Preserve the WP-09 child-wave closeout proof after umbrella `#3974` closure,
while leaving downstream demo/release-tail work explicit and non-blocking.
...
| `#3974` | WP-09 umbrella | closed / completed | Umbrella truth normalization and publication are complete; downstream work remains separately routed |
...
- The Observatory/Unity feature doc now explicitly records that O-00 through
  O-06 and the `#3974` umbrella are closed, while downstream demo/runtime
  convergence remains outside WP-09 implementation scope.
...
- `wp09_child_wave_closed_umbrella_closed`
...
- `#4341` is no longer a live WP-09 residual because the portable HTML/mobile
  lane is landed;
- umbrella `#3974` has closed after milestone docs and issue-card truth were
...
| --- | --- | --- |
| Umbrella truth normalization and publication | `#3974` | Completed; feature doc, closeout packet, demo matrix, milestone summary docs, and issue cards now record the WP-09 child wave and umbrella as closed and bounded |
...
- O-00 through O-06 are closed and bounded without hiding open downstream
  milestone work;
- umbrella publication in `#3974` is complete, and any remaining work belongs to
  downstream demo/runtime convergence rather than missing WP-09 implementation.
...
- umbrella `#3974` closed on doc/card truth rather than reopening child
