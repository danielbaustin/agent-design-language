...
| D3 HITL Pause/Resume | #402 | `SWARM_OLLAMA_BIN=swarm/tools/mock_ollama_v0_4.sh cargo run -q --manifest-path swarm/Cargo.toml --bin swarm -- swarm/examples/v0-6-hitl-pause-resume.adl.yaml --run --trace` then `SWARM_OLLAMA_BIN=swarm/tools/mock_ollama_v0_4.sh cargo run -q --manifest-path swarm/Cargo.toml --bin swarm -- resume v0-6-hitl-pause-demo` and baseline `SWARM_OLLAMA_BIN=swarm/tools/mock_ollama_v0_4.sh cargo run -q --manifest-path swarm/Cargo.toml --bin swarm -- swarm/examples/v0-6-hitl-no-pause.adl.yaml --run --out <plain_out>` | `out/s3.txt`, `<plain_out>/s3.txt`, `.adl/runs/v0-6-hitl-pause-demo/run.json` | Resume output must match non-paused output; repeat whole cycle twice. |
