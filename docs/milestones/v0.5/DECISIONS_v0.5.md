...
|----|----------|--------|-----------|-------------|--------|------|
| D-01 | Treat ADL as a first-class language with explicit schemas for all 6 primitives (`providers`, `tools`, `agents`, `tasks`, `workflows`, `run`). | accepted | Prevent runtime-driven drift and implicit structure; enable validation, tooling, editor support, and composition. | Continue implicit struct-based modeling only. | Enables schema validation, better docs, stronger demos. | #330 |
| D-02 | Separate language primitives from composition rules and later packet/control-plane contracts. | accepted | Clarifies language core vs orchestration semantics and prevents packet-schema drift from muddying the language story. | Embed composition rules and packet contracts inside one undifferentiated schema story. | Cleaner mental model and easier extension. | #330 |
| D-02a | Keep `patterns` and `signature` as top-level language features without increasing the primitive count beyond six. | accepted | Matches the corrected runtime/schema surface while preserving the canonical six-primitives model. | Reclassify every top-level feature as a new primitive. | Keeps the public language story stable for tooling and editors. | #2464 |
