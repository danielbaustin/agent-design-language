...
| Governed merge-readiness gate | `features/GOVERNED_MERGE_READINESS_GATE.md` | `python3 adl/tools/validate_merge_readiness_packet.py docs/milestones/v0.91.3/review/merge_readiness` | Merge-readiness packet preserves a structured snapshot of issue, PR, CI, branch, review, and closeout truth. | passed under #3204 with `review/merge_readiness/`, structured reconciliation JSON, and focused validator/test proof |
...
- one targeted CI proof-validation lane that runs relevant `v0.91.3` packet validators when tracked proof/demo surfaces change
