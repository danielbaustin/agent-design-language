...
| D-05 | Use tracked workflow records as the public proof target. | accepted | C-SDLC must be inspectable from Git, not only local execution cache. | Leave durable evidence in `.adl` local state. | `docs/milestones/v0.91.3/review/evidence/csdlc/` becomes the first-slice target namespace. | [README](README.md) |
