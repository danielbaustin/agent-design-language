# v0.91.7 Vision

## Metadata

- Project: ADL
- Milestone: `v0.91.7`
- Version: `v0.91.7`
- Date: `2026-06-21`
- Owner: ADL maintainers
- Setup lineage: `#3801`, `#3825`, `#4368`

## Purpose

Define the final pre-`v0.92` implementation/readiness tranche. `v0.91.7` exists so the major
remaining conceptual surfaces become tracked, reviewable, and issue-ready
before birthday activation begins.

Lifecycle note (2026-07-18): this vision is the retained milestone baseline.
It should be read with the current closeout-tail snapshot in `README.md`, not
as a claim that the milestone is still in planning.

## Overview

`v0.91.7` moves ADL from first-tranche readiness into required implementation
and integrated proof closure. It does not approve `v0.92` activation by itself.

The milestone focuses on:

- v0.91.6 closeout truth and release-tail cleanup;
- SEP, VPP, PVF, prompt-template, watcher, closeout, and sprint execution discipline;
- goal state, nested goals, and per-issue time/token/resource accounting;
- cognitive scheduler, provider suitability, and local-agent acceleration;
- build throughput, validation manager, remote/local build runners, and long-test fanout;
- runtime fire-up, Soak #2, AWS heartbeat/ACIP signal integration, and SSM/S3 archive ownership;
- Observatory/Unity/demo readiness;
- Curiosity Engine / Discovery Substrate;
- Constructability Gate;
- reasoning graph, loop runtime, and `adl.skill.v1`;
- security readiness;
- ACIP/A2A/protobuf implementation decisions;
- affect, happiness, humor, and wellbeing public claim boundaries;
- Godel mechanics for experiment/hypothesis/promotion paths;
- economics-context and guild/civilization boundary decisioning;
- launch and v0.92 birthday handoff.

## Core Goals

1. Make every second-tranche implementation requirement visible as a tracked feature doc or issue.
2. Prevent `v0.92` from absorbing unresolved conceptual work silently.
3. Preserve security and ACIP/A2A requirements on the activation path.
4. Make Curiosity and Constructability real pre-`v0.92` gates, not vague ideas.
5. Keep affect, Godel mechanics, and economics evidence bounded by safe claims.

## Non-Goals

- Do not implement runtime features.
- Do not claim `v0.92` readiness.
- Do not create broad public affect, wellbeing, or consciousness claims.
- Do not replace the `v0.92` birthday milestone.

## Success Criteria

- The 10 planning docs exist for `v0.91.7`.
- The eight second-tranche feature docs exist and cross-link from the index.
- `#3780` can refresh `v0.92` activation docs from tracked implementation/proof truth.
