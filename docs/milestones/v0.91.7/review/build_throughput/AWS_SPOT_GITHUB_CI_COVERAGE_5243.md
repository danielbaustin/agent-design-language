...
| `adl-ci-and-coverage` GitHub PR #5312 (`29357501220`) | `d0f9f06fb` | immutable digest `sha256:fbca234c9...` | retained 1000 GiB EBS; warm path | CI passed; coverage policy correctly skipped risky tests; coverage tree pruned by 24,764,862,464 bytes; verified operator `/32` SSM stdout/stderr live logs; cleanup passed; finalizer passed | 9s profile; 122s remote command | 224s | Spot check, `adl-ci`, `adl-coverage`, redaction, and cleanup all passed. Hosted slow-proof matrix remained separate and was not used for this proof. |
