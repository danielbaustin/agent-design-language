...
| `adl-ci-and-coverage` GitHub PR #5312 (`29334936665`) | `7c3585fc3` | immutable digest `sha256:20831e358...` | retained 1000 GiB EBS; warm path | Spot launch and cleanup passed; SSM command timed out at old 300s ceiling during branch-target compilation | >300s | >300s | Environment admission was fixed before this run; the next publication uses the 600s kill threshold while retaining the 300s performance target |
| `adl-ci-and-coverage` GitHub PR #5312 (`29357501220`) | `49815bec8` | immutable digest `sha256:fbca234c9...` | retained 1000 GiB EBS; warm path | CI passed; coverage policy correctly skipped risky tests; coverage tree pruned by 177,871,990,784 bytes; SSM stdout/stderr live logs; cleanup passed | 78s | 309s | Spot execution passed; the first GitHub artifact finalizer rejected the documented operator-allowlist SSM fallback as missing SSH proof; follow-up finalizer repair is required before PR checks can turn green |
