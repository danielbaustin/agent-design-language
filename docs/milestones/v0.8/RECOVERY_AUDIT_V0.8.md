...
- The repository contains real bounded v0.8 runtime/demo surfaces under `swarm/src/godel/`, `swarm/src/demo.rs`, `demos/transpiler_demo/`, `examples/`, and `demos/`.
...
- Bounded Gödel CLI surfaces under `swarm/src/cli/godel_cmd.rs`:
  - `adl godel run`
  - `adl godel inspect`
  - `adl godel evaluate`
- User-facing demo runbooks under `demos/`:
...
- Schema spine and canonical examples under `docs/milestones/v0.8/`:
  - `GODEL_EXPERIMENT_WORKFLOW_TEMPLATE_V1.md`
  - `TOOL_RESULT_CONTRACT_V1.md`
  - `experiment_index_entry.v1` schema/example docs
  - `run_summary.v1` schema/example docs
  - `agent_profile.v1` schema docs
  - `CANONICAL_EVIDENCE_VIEW_V1.md`, `MUTATION_FORMAT_V1.md`, `EVALUATION_PLAN_V1.md`, and `EXPERIMENT_RECORD_V1.md` now describe bounded runtime-integrated surfaces as well as their canonical contracts
...
### Observed facts
- The following bounded runtime artifacts are now persisted and reviewed through executable or CLI-backed paths:
  - `experiment_record.v1`
  - `canonical_evidence_view.v1`
  - `mutation.v1`
  - `evaluation_plan.v1`
  - `tool_result.v1`
- `godel_experiment_workflow.template.v1` and `experiment_index_entry.v1` remain narrower review/contract surfaces than the artifacts listed above.
- `docs/milestones/v0.8/DEMOS_V0.8.md` and `docs/milestones/v0.8/QUALITY_GATE_V0.8.md` remain reviewer/planning surfaces rather than release-candidate declarations.
...
- Reviewer-entry docs still require explicit run-vs-inspect guidance to avoid confusion about which surfaces are runnable demos versus inspect-only review artifacts.

### Inferred conclusions
- Remaining v0.8 gaps are about handoff packaging, clarity, and the final bounded/deferred distinction, not about proving the milestone has zero executable runtime value.
- Demo and quality-gate surfaces are documented and exercised, but they are still not the same thing as a final release-candidate gate declaration.
- The primary remaining gaps are final review-packet clarity, third-party handoff packaging, and release-tail closeout.
...
- `swarm/README.md` says:
  - latest tagged runtime release: v0.7.0
  - current main-branch crate version: 0.8.0
  - this is unreleased v0.8 development documentation layered on top of the latest tagged release
...
### Inferred conclusions
- The coherent version story is available and is already present in the root README, runtime README, and manifest.
- The remaining review-tail work is less about version contradiction and more about preserving that story consistently in the external handoff packet.
