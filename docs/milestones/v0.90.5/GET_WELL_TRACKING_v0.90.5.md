...
| GW-03 | Collapsed the delegation-subcontract focused default surface from `7` sibling tests to `2` aggregate tests, with `4` aggregate tests under `slow-proof-tests`. | `cargo test --manifest-path adl/Cargo.toml runtime_v2_delegation_subcontract -- --nocapture` and `cargo test --manifest-path adl/Cargo.toml --features slow-proof-tests runtime_v2_delegation_subcontract -- --nocapture` both passed locally. | Authoritative wall-time effect remains pending until GitHub `adl-coverage` runs this PR. |
| GW-04 | Collapsed the contract-market demo lib surface from `4` focused tests to `2` aggregate tests while folding the negative-packet denial check into the focused demo filter, and collapsed the resource-stewardship bridge from `4` default tests to `2` aggregate tests, with `3` aggregate tests under `slow-proof-tests`. | `cargo test --manifest-path adl/Cargo.toml runtime_v2_contract_market_demo -- --nocapture`, `cargo test --manifest-path adl/Cargo.toml --features slow-proof-tests runtime_v2_contract_market_demo -- --nocapture`, `cargo test --manifest-path adl/Cargo.toml runtime_v2_resource_stewardship_bridge -- --nocapture`, and `cargo test --manifest-path adl/Cargo.toml --features slow-proof-tests runtime_v2_resource_stewardship_bridge -- --nocapture` passed locally. | Authoritative wall-time effect remains pending until GitHub `adl-coverage` runs this PR. |
| GW-05 | Replaced the CLI feature-proof regression-matrix rerun with a cheap registry assertion so it no longer recursively invokes sibling CLI/demo tests, while preserving the individual D13 feature-proof CLI smoke and D12 contract-market CLI argument/path validation tests. | `cargo test --manifest-path adl/Cargo.toml runtime_v2_feature_proof_coverage -- --nocapture` passed locally, and `cargo nextest run --manifest-path adl/Cargo.toml --status-level all --final-status-level slow -E 'test(runtime_v2_feature_proof_coverage) or test(runtime_v2_contract_market_demo_validates_stdout_help_and_output_path_rules)'` passed `7` focused tests in `3.436s`. | Authoritative wall-time effect remains pending until GitHub `adl-coverage` runs this PR; local focused nextest confirms the former matrix test itself is no longer a slow tail. |
