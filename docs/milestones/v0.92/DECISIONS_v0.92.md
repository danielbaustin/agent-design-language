...
| D-08 | v0.92 owns ACIP binary schema, public schema catalog, JSON projection, and authenticated full-duplex Runtime WebSocket carrier readiness. | Accepted for planning | ACIP is already a local communication substrate, but citizen/polis communication needs a binary/protobuf shape that remains publicly decodeable by schema and governed by access rules. | Keeps the communication layer inspectable while deferring distributed cross-polis key lifecycle to v0.93 and signed/queryable trace closure to v0.94. |
| D-09 | v0.92 WP-01 must consume `v0.91.5` release-tail closeout, the `v0.91.6` readiness tranche, the `v0.91.7` implementation/proof tranche, and `#3377` before seeding final issues. | Accepted for planning | First-birthday readiness now depends on the full pre-`v0.92` readiness/proof sequence rather than `v0.91.5` alone. | Prevents the milestone from reconstructing birthday readiness from chat or duplicating the `#3377` packet. |
...
- Any expansion into personhood, production citizenship, governance,
  ...
  implementation.
- Any expansion from the v0.92 authenticated Runtime carrier into distributed
  cross-polis key lifecycle or mesh-wide transport-security authority is
  deferred to later milestone decisions.
