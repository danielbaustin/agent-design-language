...
| D-03 | Introduce a **bounded affect model** as a reasoning control surface. | accepted | Bounded affect signals help prioritize reasoning and evaluation without introducing anthropomorphic claims. | Ignore affect entirely or implement unconstrained cognitive features. | Provides a structured foundation for later reasoning systems. | `AFFECT_MODEL_v0.85.md` |
...
| D-09 | Execute the final **`swarm` -> `adl` active-surface cutover at the end of v0.85**, after other code/review work stabilizes. | accepted | The cutover is path-sensitive and repo-wide; running it late reduces branch drift and merge-conflict pressure while substantive milestone work is still landing. | Perform the cutover earlier in parallel with feature work, or defer it to a later milestone again. | Makes the cutover a deliberate release-tail package instead of background churn. | `SWARM_REMOVAL_PLANNING.md` |
| D-10 | Record a final **Go** decision for the v0.85 release ceremony. | accepted | Internal review, external review, remediation, quality-gate verification, demo closeout, and canonical milestone-doc reconciliation have all been completed to a release-ready standard. Remaining work is release mechanics rather than milestone substance. | Delay release until further roadmap or architecture work lands; continue refining v0.85 docs indefinitely before tagging. | Authorizes `v0.85.0` tagging and GitHub Release creation once final release mechanics are executed. | `RELEASE_PLAN_v0.85.md` |
