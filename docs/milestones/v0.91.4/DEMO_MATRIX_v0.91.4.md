...
| D15 | Best-demo showcase, including Creative Room and Unity-facing C-SDLC proof | WP-13 packages the strongest reviewer-facing demos, including Creative Room and a Unity-facing demo if available. | `demos/v0.91.4/adl_creative_room_demo.html` plus WP-owned demo showcase runbook | `docs/milestones/v0.91.4/review/demo_showcase/CREATIVE_ROOM_PROOF_PACKET_v0.91.4.md`, `docs/milestones/v0.91.4/review/demo_showcase/STARHARVEST_REFRESH_NOTE_v0.91.4.md`, plus later showcase packet and proof index | demos are inspectable, bounded, and do not overclaim release state | Creative Room is a static/browser-rendered front-stage artifact; Starharvest reuses the v0.91.3 artifact with fresh browser proof; Unity remains separately owned and last | in_progress |
...
| D17 | Multi-agent C-SDLC workcell proof | MA-CSDL-00 through MA-CSDL-04 prove bounded conductor-managed parallel worker/reviewer/janitor lanes without losing card, branch, PR, validation, review, or closeout truth. | workcell assignment planner and proof runbook | workcell model, assignment plan, state packet, proof review packet, finding disposition, and Codex-only follow-on slice | two local Ollama worker lanes and one hosted Codex reviewer lane were coordinated safely after serialized publication normalization; a later ad hoc Codex-only slice also showed two hosted Codex worker lanes succeeding in parallel on disjoint shards | bounded proof; live parallelism only where write sets and dependencies are disjoint | in_progress |
