...
| D15 | Best-demo showcase, including Unity-facing C-SDLC proof | WP-13 packages the strongest reviewer-facing demos, including a Unity-facing demo if available. | WP-owned demo showcase runbook | `docs/milestones/v0.91.4/review/demo_showcase/STARHARVEST_REFRESH_NOTE_v0.91.4.md` plus later showcase packet and proof index | demos are inspectable, bounded, and do not overclaim release state | Starharvest reuses the v0.91.3 artifact and now has fresh browser proof for load, render, interaction, and restart | in_progress |
