...
| Runtime inhabitant integration proof | WP-15 | Show standing, state, lifecycle, memory, comms, capability, learning, and observatory surfaces integrated into one agent-shaped run surface. | `adl/src/runtime_v2/runtime_inhabitant_integration.rs`, deterministic operator report, integrated run packet, and trace/projection/access refs | does not claim birthday, full identity continuity, or flagship completeness |
