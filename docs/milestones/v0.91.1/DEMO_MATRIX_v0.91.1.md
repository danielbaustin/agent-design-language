...
| Runtime inhabitant integration proof | WP-15 | Show standing, state, lifecycle, memory, capability, comms, learning, and observatory surfaces integrated into one agent-shaped run surface. | `adl/src/runtime_v2/runtime_inhabitant_integration.rs`, deterministic operator report, integrated run packet, `docs/milestones/v0.91.1/review/capability_aptitude_testing_fixture/scorecard.json`, and trace/projection/access refs | does not claim birthday, full identity continuity, or flagship completeness |
| Observatory-visible agent flagship demo | WP-16 | Show an agent-shaped run inside the CSM boundary with operator projection and explicit feature-demo coverage for every landed `v0.91.1` feature. | `adl runtime-v2 observatory-flagship-demo --out artifacts/v0911/demo-d12-observatory-flagship`, `adl/src/runtime_v2/observatory_flagship.rs`, and the tracked review bundle under `docs/milestones/v0.91.1/review/observatory_flagship_demo/`; the flagship proof packet now directly binds the lifecycle state contract and transition matrix, the ACIP hardening packet, the A2A adapter boundary packet, the runtime inhabitant integration packet, and a feature-by-feature coverage roster for WP-02 through WP-16 | does not claim birthday, personhood, raw private-state inspection, or autonomous federation |
