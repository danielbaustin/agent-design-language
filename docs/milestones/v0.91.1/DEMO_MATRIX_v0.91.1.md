...
| ACIP/A2A hardening demo | WP-13, WP-14 | Show authenticated local comms, redaction, adapter boundary, and state-gated invocation. | `adl/src/runtime_v2/acip_hardening.rs` plus ACIP conformance and lifecycle proof surfaces; packet path is generated as `runtime_v2/acip/acip_hardening_packet.json` | does not claim external transport readiness |
