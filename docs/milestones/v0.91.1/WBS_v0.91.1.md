...
| C | Agent lifecycle state model | Implement the bounded consciousness/lifecycle state contract, transition matrix, and per-state ACIP reception/invocation rules. | lifecycle state contract and fixtures | B |
| D | CSM observatory active surface | Turn observatory planning into active packets, projections, and operator-visible runtime state. | observatory active-surface implementation | B, C |
| E | Citizen standing | Implement and document standing classes, transitions, and naked-actor rejection boundaries. | citizen-standing contract and fixtures | B, C |
| F | Citizen state | Harden citizen-state format, security, projection, and review boundaries. | citizen-state substrate update | C, E |
| G | Memory and identity architecture | Prepare memory/identity evidence surfaces without claiming full v0.92 identity continuity. | memory/identity architecture and fixtures | F |
| H | Theory of Mind foundation | Model bounded agent-state hypotheses and update events tied to evidence. | ToM contract, schemas, and fixtures | F, G |
| I | Capability and aptitude testing | Create the first executable capability/aptitude harness and report model. | capability test harness slice | B, C, F |
| J | Intelligence metric architecture | Define intelligence metrics as evidence-bound architecture, not reputation or mystique. | metric architecture and fixture report | I |
| K | Governed learning substrate | Bound learning updates, feedback, and adaptation evidence under policy. | governed-learning contract | G, I, J |
| L | ANRM/Gemma placement | Place ANRM/Gemma as a bounded local-model evidence and trace-dataset lane. | ANRM placement package and extractor spec | I, J, K |
| M | ACIP/A2A hardening | Harden local comms envelopes, redaction, invocation, conformance, and A2A adapter boundaries. | secure comms hardening slice | C, F |
| N | Runtime inhabitant integration | Integrate standing, state, memory, comms, capability, learning, lifecycle state, and observatory into an agent-shaped run. | inhabitant runtime integration | D through M |
...
Runtime/polis architecture must land before the feature slices widen. The agent
lifecycle state model must land before ACIP, Observatory, memory/identity, and
runtime-inhabitant integration rely on state-specific agency claims. Citizen
