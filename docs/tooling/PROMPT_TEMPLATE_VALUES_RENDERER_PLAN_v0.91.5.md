...
object with static validation, locked system fields, enum checks, and explicit
failure modes. This reduces the card churn seen during v0.91.4/v0.91.5 setup
without changing C-SDLC lifecycle semantics.
...
The static validator should reject:
...
- invalid enum value;
- non-string value where the template requires text;
- non-list value where the schema requires list items;
...
- downstream card rewrite after v1.1 is tracked through `#3582`;
...
follow-on is downstream card rewrite/normalization after the renderer lands
(`#3582`).
...
- values validation rejects the failure modes above;
