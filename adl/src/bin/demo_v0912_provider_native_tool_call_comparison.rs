...
fn resolve_out_path(arg: Option<String>) -> PathBuf {
    arg.map(PathBuf::from).unwrap_or_else(|| {
        PathBuf::from(
            adl::provider_native_tool_call_comparison::PROVIDER_NATIVE_TOOL_CALL_COMPARISON_REPORT_ARTIFACT_PATH,
        )
    })
}

fn write_report(path: &PathBuf) -> Result<()> {
    adl::provider_native_tool_call_comparison::write_provider_native_tool_call_comparison_report(
        path,
    )
    .with_context(|| {
        format!(
            "write provider-native tool-call comparison report '{}'",
            path.display()
        )
    })?;
    Ok(())
...
mod tests {
    ...
    #[test]
    fn demo_v0912_provider_native_tool_call_comparison_resolve_out_path_uses_explicit_argument() {
        let path = resolve_out_path(Some(
            "tmp/provider-native-tool-call-comparison-report.json".to_string(),
        ));
        assert_eq!(
            path,
            std::path::PathBuf::from("tmp/provider-native-tool-call-comparison-report.json")
        );
    }

    #[test]
    fn demo_v0912_provider_native_tool_call_comparison_resolve_out_path_defaults_to_tracked_artifact_path(
    ) {
    ...
    #[test]
    fn demo_v0912_provider_native_tool_call_comparison_write_report_creates_expected_json_artifact()
    {
        let path = unique_temp_path("provider-native-tool-call-comparison-bin");
