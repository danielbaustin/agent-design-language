...
usage() {
  cat <<'EOF'
Usage:
  adl/tools/demo_one_command.sh [--demo <name>] [--out <dir>] [--trace] [--print-plan]

Default behavior:
- Runs the recommended demo UX path (`demo-b-one-command`)
- Writes artifacts under `.adl/reports/demo`
- Uses quiet mode and auto-open behavior from `adl demo`
EOF
}
...
args=(run --manifest-path "$ROOT/adl/Cargo.toml" -- demo "$DEMO_NAME")
