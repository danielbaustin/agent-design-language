...
(
  cd "$ROOT_DIR"
  bash adl/tools/demo_v0871_provider_mock.sh "$ARTIFACT_ROOT" >/dev/null
)
...
grep -Fq '"run_id": "v0-87-1-provider-mock-demo"' "$SUMMARY_FILE" || {
...
grep -Fq 'MOCK_PROVIDER_DEMO_OK' "$LOG_FILE" || {
  echo "assertion failed: run_log.txt missing mock output" >&2
  exit 1
}
grep -Fq 'demo_v0871_provider_mock.sh' "$README_FILE" || {
  echo "assertion failed: README missing wrapper reference" >&2
  exit 1
}

echo "demo_v0871_provider_mock: ok"
