...
source "$ROOT/adl/tools/card_paths.sh"
...
Examples:
  ...
  adl/tools/open_artifact.sh burst latest
EOF
}
...
main() {
  ...
  case "$kind" in
    card)
      ...
      case "$which" in
        input) p="$(resolve_input_card_path "$issue" "")" ;;
        output) p="$(resolve_output_card_path "$issue" "")" ;;
