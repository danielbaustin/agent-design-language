...
if [[ -n "$TOOLING_BIN" ]]; then
  exec "$TOOLING_BIN" tooling verify-review-output-provenance "$@"
fi
exec cargo run --quiet --manifest-path "$ROOT_DIR/adl/Cargo.toml" --bin adl -- tooling verify-review-output-provenance "$@"
