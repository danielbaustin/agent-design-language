...
MOCK_BIN="$ROOT/adl/tools/mock_ollama_v0_4.sh"
...
run_demo() {
  ...
  echo "==> Running $name"
  cargo run -q --manifest-path "$ROOT/adl/Cargo.toml" --bin adl -- "$yaml" --run --trace --out "$out_dir"
}

run_demo "fork-join" "$ROOT/adl/examples/v0-4-demo-fork-join.adl.yaml"

bounded_start="$(date +%s)"
run_demo "bounded-parallelism" "$ROOT/adl/examples/v0-4-demo-bounded-parallelism.adl.yaml"
bounded_end="$(date +%s)"
echo "bounded-parallelism elapsed_seconds=$((bounded_end - bounded_start))"

run_demo "deterministic-replay-pass-1" "$ROOT/adl/examples/v0-4-demo-deterministic-replay.adl.yaml"
run_demo "deterministic-replay-pass-2" "$ROOT/adl/examples/v0-4-demo-deterministic-replay.adl.yaml"
