# Repo Review Code Skill Input Schema

```yaml
skill_input_schema: repo_review_code.v1
...
Use this schema when invoking `repo-review-code` as a bounded code/correctness specialist in the multi-agent repository review suite. The skill focuses on behavioral bugs, regressions, maintainability risks, API misuse, state transitions, concurrency, parsing, serialization, and implementation drift.
...
```text
.adl/reviews/<timestamp>-repo-review-code.md
```

The artifact must follow the local output contract at:

```text
adl/tools/skills/repo-review-code/references/output-contract.md
```
