# Repo Review Tests Skill Input Schema

```yaml
skill_input_schema: repo_review_tests.v1
...
Use this schema when invoking `repo-review-tests` as a bounded test-quality specialist in the multi-agent repository review suite. The skill focuses on missing coverage, weak assertions, brittle fixtures, validation gaps, test isolation, flaky tests, and risky behavior without executable proof.
...
```text
.adl/reviews/<timestamp>-repo-review-tests.md
```

The artifact must follow the local output contract at:

```text
adl/tools/skills/repo-review-tests/references/output-contract.md
```
