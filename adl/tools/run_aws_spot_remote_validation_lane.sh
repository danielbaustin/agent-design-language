...
REMOTE_COMMAND_TIMEOUT_SECONDS="${ADL_AWS_REMOTE_VALIDATION_COMMAND_TIMEOUT_SECONDS:-300}"
...
if [[ ! "$REMOTE_COMMAND_TIMEOUT_SECONDS" =~ ^[0-9]+$ ]] || (( REMOTE_COMMAND_TIMEOUT_SECONDS < 1 || REMOTE_COMMAND_TIMEOUT_SECONDS > 300 )); then
  echo "run_aws_spot_remote_validation_lane: command timeout must be between 1 and 300 seconds" >&2
  exit 2
fi
