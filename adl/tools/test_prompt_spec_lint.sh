...
LINT_SCRIPT="$ROOT_DIR/adl/tools/lint_prompt_spec.sh"
...
repo="$tmpdir/repo"
mkdir -p "$repo/adl/tools" "$repo/.adl/cards/761"
cp "$LINT_SCRIPT" "$repo/adl/tools/lint_prompt_spec.sh"
cp "$CARD_PATHS_LIB" "$repo/adl/tools/card_paths.sh"
chmod +x "$repo/adl/tools/lint_prompt_spec.sh"
...
(
  cd "$repo"
  set +e
  ./adl/tools/lint_prompt_spec.sh --input .adl/cards/761/input_invalid_761.md >/dev/null 2>&1
  rc=$?
  set -e
  if [[ "$rc" -eq 0 ]]; then
...
(
  cd "$repo"
  set +e
  ./adl/tools/lint_prompt_spec.sh --input .adl/cards/761/input_missing_surface_761.md >/dev/null 2>&1
  rc=$?
  set -e
  if [[ "$rc" -eq 0 ]]; then
