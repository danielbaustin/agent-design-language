...
if [[ -n "$TOOLING_BIN" ]]; then
  exec "$TOOLING_BIN" tooling verify-repo-review-contract "$@"
fi
exec cargo run --quiet --manifest-path "$ROOT_DIR/adl/Cargo.toml" --bin adl -- tooling verify-repo-review-contract "$@"
