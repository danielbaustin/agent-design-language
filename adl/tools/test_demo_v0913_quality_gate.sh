...
(
  cd "$ROOT_DIR"
  ADL_V0913_QUALITY_GATE_ONLY_CHECKS=quality_gate_doc_surface,quality_gate_packet_surface,demo_coverage_surface,demo_matrix_wp11_row \
    bash adl/tools/demo_v0913_quality_gate.sh "$OUT_DIR" >/dev/null
)
...
if manifest.get("demo_id") != "D19":
    raise SystemExit("unexpected quality gate demo id")
if manifest.get("manifest_version") != "adl.v0913.quality_gate.v1":
...
required = {
    ...
    "demo_coverage_surface",
    "demo_matrix_wp11_row",
}
...
for heavy in ("fmt", "clippy"):
    check = checks.get(heavy, {})
    if check.get("status") != "SKIP":
        raise SystemExit(f"expected heavy check {heavy} to be skipped")
PY

grep -Fq 'does not by itself declare `v0.91.3` docs-reviewed, internally reviewed, externally reviewed, or release-ready' "$README" || {
...
if (
  cd "$ROOT_DIR"
  ADL_V0913_QUALITY_GATE_ONLY_CHECKS=quality_gate_doc_surface \
    ADL_V0913_QUALITY_GATE_FORCE_FAIL_CHECKS=quality_gate_doc_surface \
    bash adl/tools/demo_v0913_quality_gate.sh "$FAIL_OUT_DIR" >/dev/null 2>"$TMPDIR_ROOT/fail.stderr"
); then
...
(
  cd "$ROOT_DIR"
  ADL_V0913_QUALITY_GATE_INSPECT_ONLY=1 \
    ADL_V0913_QUALITY_GATE_ONLY_CHECKS=quality_gate_doc_surface \
    ADL_V0913_QUALITY_GATE_FORCE_FAIL_CHECKS=quality_gate_doc_surface \
    bash adl/tools/demo_v0913_quality_gate.sh "$INSPECT_OUT_DIR" >/dev/null 2>"$TMPDIR_ROOT/inspect.stderr"
)
...
CUSTOM_OUT_DIR="$TMPDIR_ROOT/custom-root"
(
  cd "$ROOT_DIR"
  ADL_V0913_QUALITY_GATE_ONLY_CHECKS=quality_gate_doc_surface \
    bash adl/tools/demo_v0913_quality_gate.sh "$CUSTOM_OUT_DIR" >/dev/null
)

python3 - "$CUSTOM_OUT_DIR/quality_gate_record.json" <<'PY'
import json
import sys

manifest = json.load(open(sys.argv[1], encoding="utf-8"))
for key, check in manifest.get("checks", {}).items():
    log = check.get("log", "")
    if log.startswith("/"):
        raise SystemExit(f"check {key} leaked absolute log path: {log}")
PY

echo "demo_v0913_quality_gate: ok"
