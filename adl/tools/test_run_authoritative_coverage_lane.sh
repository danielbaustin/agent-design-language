...
for required in \
  "cmd=llvm-cov nextest --workspace --no-report --no-clean --no-fail-fast --no-tests pass" \
  "--test-threads 4" \
  "--partition count:1/2" \
  "--partition count:2/2" \
  "-- --skip real_pr_" \
  "cmd=llvm-cov nextest --manifest-path $ROOT_DIR/adl-runtime/Cargo.toml --no-report --no-clean --no-fail-fast --no-tests pass" \
