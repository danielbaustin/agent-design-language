...
def preparation_validator_error(path, argv:, behavior_probe: false, planned_output: false)
  ...
  return "preparation validator terminated by signal instead of failing closed: #{path}" if probe_status.signaled?

  crash_markers = %r{
    (?:
      Traceback\s+\(most\s+recent\s+call\s+last\)
      |(?:^|\n).*?:\d+:in\s+[`']
      |\((?:Errno::[A-Za-z0-9_]+|JSON::ParserError|KeyError|LoadError|NameError|NoMethodError|SyntaxError|SystemCallError|TypeError)\)
      |undefined\s+method
      |uninitialized\s+constant
...
puts "v0.92 readiness: PASS (#{rows.length} design-ready issues across #{sprints.length} sprints; #{behavior_probes}/#{script_contracts} script contracts invocation/fail-closed probed, #{deferred_execution_scripts} deferred to child execution)"

if matrix_path
  lines = [
    "# v0.92 Execution-Issue Readiness Matrix",
    "",
    "Generated by `.csdlc/prepared/issues/5860/validate-v092-readiness.rb` after full structural, dependency, path, digest-projection, artifact-integrity, and issue-local validator invocation/fail-closed validation.",
    "",
  ...
  lines.concat([
    ...
    "",
    "Validator contract proof: #{behavior_probes} of #{script_contracts} VPP script contracts were invocation/fail-closed probed during live readiness validation; #{deferred_execution_scripts} implementation or evidence-production scripts remain explicitly deferred to their child execution lanes. A probe passes only when the script exits successfully or rejects the pre-execution fixture with a deliberate diagnostic and no uncaught exception, backtrace, shell execution fault, or signal termination.",
    "",
