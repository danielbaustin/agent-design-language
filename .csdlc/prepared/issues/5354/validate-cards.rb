...
assert(registry.fetch("schema") == "adl.csdlc.prompt_template_registry.v1", "registry schema mismatch")
...
index_path = ROOT.join(".csdlc/issues/5354/index.json")
assert(index_path.file?, "missing typed index .csdlc/issues/5354/index.json")
index = JSON.parse(index_path.read)
assert(index.fetch("cards").keys.sort == CARDS.sort, "typed card inventory mismatch")
CARDS.each do |name|
  card = ROOT.join(".csdlc/issues/5354/cards/#{name}.md")
  values = ROOT.join(".csdlc/issues/5354/cards/#{name}.values.json")
  assert(card.file? && values.file?, "missing #{name} card/value pair")
  parsed = JSON.parse(values.read)
  assert(parsed.dig("identity", "issue") == 5354, "wrong #{name} issue")
  assert(parsed.dig("content", "card_kind") == name, "wrong #{name} kind")
end

out, status = Open3.capture2e(doctor.to_s, "--repo", ".", "--issue", "5354", chdir: ROOT.to_s)
...
puts JSON.generate(status: "pass", issue: 5354, registry: registry.fetch("csdlc_prompt_template_set"), native_template: native.fetch("template_set"), cards: CARDS.length)
