...
generated_at: "2026-07-02T17:59:01Z"
card_status: "ready"
status: "ready"
...
notes: "Planning corrected from generated docs-only profile to first-class lifecycle shepherd tooling slice. Focus proof on the new repo-native `adl-pr-shepherd` owner binary, its doctor/watch synthesis, fail-closed compatibility-wrapper owner-binary enforcement, and refreshed-base coexistence with the upstream `pr-inventory` command surface. After PR publication, `adl-ci` failed in the retained v0.91.3 proof-validation lane because the older card-lifecycle contract commands broad-compiled enough of the tree to exhaust runner disk. Validation planning now explicitly includes the blocker-driven exact `--bin adl` retained-proof contract tests, the authoritative coverage runner contract, and the focused delegate contract script that proves the wrapper no longer falls through to a stale generic `adl` binary."
---
