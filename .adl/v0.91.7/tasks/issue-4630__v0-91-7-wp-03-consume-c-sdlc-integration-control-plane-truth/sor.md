...
Card Status: ready
Status: DONE
Generated: 2026-07-02T17:35:41Z
...
- Ran focused validation for the owner binary lane, `git diff --check`, direct `adl-pr-shepherd` JSON output, and compatibility-wrapper `pr.sh shepherd` JSON output.
...
- Integration method used: issue-bound worktree implementation published to draft PR `#4714`, then janitored in place with blocker-driven proof-lane narrowing on commit `f5115e894f4b7cb081f67ac79d66081684d652b2`
...
- Verification performed:
  ...
  - `ADL_GITHUB_TOKEN_FILE=$HOME/keys/github.token ./adl/tools/pr.sh shepherd 4630 --version v0.91.7 --json`
    Verified the compatibility wrapper delegates to the owner binary and returns the same lifecycle-shepherd packet shape.
  - `ADL_GITHUB_TOKEN_FILE=$HOME/keys/github.token ./adl/tools/pr.sh watch 4630 --json`
    Verified the published branch now reports `classification=pr_open`, `tail_owner=issue-watcher`, and pending GitHub checks on PR `#4714` after the janitor push.
- Result: FAIL
...
- Validation commands and their purpose:
  ...
  - `ADL_GITHUB_TOKEN_FILE=$HOME/keys/github.token ./adl/tools/pr.sh shepherd 4630 --version v0.91.7 --json`
    Proved the shell compatibility wrapper reaches the owner binary successfully and preserves the authoritative packet shape.
  - `ADL_GITHUB_TOKEN_FILE=$HOME/keys/github.token ./adl/tools/pr.sh watch 4630 --json`
    Proved the PR-tail classifier now reports `pr_open` / watcher ownership with fresh checks pending on the janitored commit.
- Results:
  ...
  - PASS: wrapper `pr.sh shepherd` JSON output returned the same authoritative packet shape and state.
...
sor_facts:
  ...
  finish:
    ...
    blocking_notes:
    - Fresh `adl-ci` and `adl-coverage` results are pending on commit `f5115e894f4b7cb081f67ac79d66081684d652b2`.
    fix_notes:
...
- Determinism tests executed: focused command proofs only; the new `adl-pr-shepherd` owner binary was exercised directly and through the compatibility wrapper, but no same-input replay bundle was captured in this issue.
...
- Replay verification (same inputs -> same artifacts/order): not formally replay-verified beyond matching direct-binary and compatibility-wrapper packet shape for the same bound issue.
...
- Artifact stability notes: implementation is now published to draft PR `#4714`; merge has not happened yet, and the current pending-check state is tied to commit `f5115e894f4b7cb081f67ac79d66081684d652b2`.
...
- Missing/optional artifacts and rationale: merged/main-integrated artifacts are intentionally absent because PR `#4714` is still open and fresh checks are pending on commit `f5115e894f4b7cb081f67ac79d66081684d652b2`.
