...
- `v0-3-concurrency-fork-join.adl.yaml`
  ...
  - concurrency cap precedence for concurrent workflow runs:
    - `run.workflow.max_concurrency` (or `workflows.<id>.max_concurrency` via `workflow_ref`)
    - `run.defaults.max_concurrency`
    - runtime default `4`
  - set cap to `1` for fully sequential execution behavior
