...
use ::adl::{
    adl, artifacts, demo, env_compat, execute, failure_taxonomy, godel, instrumentation,
    learning_export, obsmem_demo, overlay, plan, prompt, resolve, signing, trace,
};
...
fn real_main() -> Result<()> {
    ...
    if matches!(args.first().map(|s| s.as_str()), Some("godel")) {
        return real_godel(&args[1..]);
    }
...
fn real_godel(args: &[String]) -> Result<()> {
    let Some(cmd) = args.first().map(|s| s.as_str()) else {
        return Err(anyhow::anyhow!(
...
fn real_godel_run(args: &[String]) -> Result<()> {
