...
mod tests {
    ...
    #[test]
    fn effective_prompt_with_defaults_prefers_step_prompt() {
        let mut doc = minimal_resolved().doc;
        ...
        let effective =
            effective_prompt_with_defaults_from_doc(&step, &doc).expect("step prompt expected");
        assert_eq!(effective.user.as_deref(), Some("step-user"));
        assert_eq!(effective.system.as_deref(), Some("default-system"));
    }

    #[test]
    fn effective_prompt_with_defaults_uses_task_prompt_before_agent_prompt() {
        let mut doc = minimal_resolved().doc;
        doc.run.defaults.system = Some("default-system".to_string());
        doc.tasks.insert(
            "task".to_string(),
        ...
        let mut step = minimal_step("s1");
        step.task = Some("task".to_string());
        step.agent = Some("agent".to_string());
        let effective =
            effective_prompt_with_defaults_from_doc(&step, &doc).expect("task prompt expected");
        assert_eq!(effective.user.as_deref(), Some("task-user"));
        assert_eq!(effective.system.as_deref(), Some("default-system"));
    }

    #[test]
    fn effective_prompt_with_defaults_uses_agent_prompt_when_task_missing() {
        let mut doc = minimal_resolved().doc;
        doc.agents.insert(
            "agent".to_string(),
            crate::adl::AgentSpec {
        ...
        step.agent = Some("agent".to_string());
        let effective =
            effective_prompt_with_defaults_from_doc(&step, &doc).expect("agent prompt expected");
        assert_eq!(effective.user.as_deref(), Some("agent-user"));
    }

    #[test]
    fn effective_prompt_with_defaults_returns_none_without_sources() {
        let doc = minimal_resolved().doc;
        let step = minimal_step("s1");
        assert!(effective_prompt_with_defaults_from_doc(&step, &doc).is_none());
    }

    #[test]
    fn delegation_trace_target_uses_remote_action_and_fallback_provider() {
        let doc = minimal_resolved().doc;
        let mut step = minimal_step("s1");
        step.placement = Some(crate::adl::PlacementMode::Remote);
        step.delegation = Some(crate::adl::DelegationSpec::default());
        let (action, target) = delegation_trace_target(&step, &doc).expect("delegation target");
        assert_eq!(action, "remote_exec");
        assert_eq!(target, "<unresolved-provider>");
    }

    #[test]
    fn emit_delegation_lifecycle_records_expected_trace_events() {
        let doc = minimal_resolved().doc;
        let mut step = minimal_step("s1");
        step.provider = Some("provider-1".to_string());
        step.delegation = Some(crate::adl::DelegationSpec::default());
        let mut tr = Trace::new("run", "wf", "0.3");

        emit_delegation_lifecycle_start(&mut tr, &step, &doc);
        emit_delegation_lifecycle_finish(&mut tr, &step, &doc, true, 12);

        assert!(matches!(
            tr.events[0],
            crate::trace::TraceEvent::DelegationRequested { .. }
        ));
        assert!(matches!(
            tr.events[1],
            crate::trace::TraceEvent::DelegationPolicyEvaluated { .. }
        ));
        assert!(matches!(
            tr.events[2],
            crate::trace::TraceEvent::DelegationDispatched { .. }
        ));
        assert!(matches!(
            tr.events[3],
            crate::trace::TraceEvent::DelegationResultReceived { .. }
        ));
        assert!(matches!(
            tr.events[4],
            crate::trace::TraceEvent::DelegationCompleted { .. }
        ));
    }

    #[test]
    fn scheduler_policy_for_run_returns_none_for_sequential_plan() {
        let mut resolved = minimal_resolved();
        resolved.execution_plan.workflow_kind = WorkflowKind::Sequential;
        assert!(scheduler_policy_for_run(&resolved)
            .expect("policy")
            .is_none());
    }

    #[test]
    fn scheduler_policy_for_run_returns_workflow_override_for_concurrent_plan() {
        let mut resolved = minimal_resolved();
        resolved.execution_plan.workflow_kind = WorkflowKind::Concurrent;
        resolved.doc.run.defaults.max_concurrency = Some(2);
        resolved
            .doc
            .run
            .workflow
        ...
        let (max, source) = scheduler_policy_for_run(&resolved)
            .expect("policy")
            .expect("concurrent policy");
        assert_eq!(max, 3);
        assert_eq!(source, SchedulerPolicySource::WorkflowOverride);
    }

    #[test]
    fn enforce_delegation_policy_for_step_actions_denies_provider_call() {
        let mut doc = minimal_resolved().doc;
        doc.run.delegation_policy = Some(crate::adl::DelegationPolicySpec {
            default_allow: false,
            rules: vec![],
        });
        let mut step = minimal_step("s1");
        ...
        step.delegation = Some(crate::adl::DelegationSpec::default());
        let mut tr = Trace::new("run", "wf", "0.3");
        let err = enforce_delegation_policy_for_step_actions(&mut tr, &step, &doc)
            .expect_err("provider call should be denied");
        let rendered = err.to_string();
        assert!(rendered.contains(DELEGATION_POLICY_DENY_CODE));
        assert!(rendered.contains("provider_call"));
    }

    #[test]
    fn enforce_delegation_policy_for_step_actions_checks_filesystem_read() {
        let mut doc = minimal_resolved().doc;
        doc.run.delegation_policy = Some(crate::adl::DelegationPolicySpec {
            default_allow: true,
            rules: vec![crate::adl::DelegationPolicyRuleSpec {
                id: "deny-read".to_string(),
                action: crate::adl::DelegationActionKind::FilesystemRead,
                target_id: Some("@file".to_string()),
                effect: crate::adl::DelegationRuleEffect::Deny,
        ...
        step.delegation = Some(crate::adl::DelegationSpec::default());
        step.inputs
            .insert("doc".to_string(), "@file:inputs.md".to_string());
        let mut tr = Trace::new("run", "wf", "0.3");
        let err = enforce_delegation_policy_for_step_actions(&mut tr, &step, &doc)
            .expect_err("filesystem read should be denied");
        let rendered = err.to_string();
        assert!(rendered.contains("filesystem_read"));
        assert!(rendered.contains("@file"));
    }

    #[test]
    fn resume_disposition_for_step_reruns_when_artifact_is_missing() {
        let now = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .expect("time")
            .as_nanos();
        let out_dir =
            std::env::temp_dir().join(format!("swarm-resume-missing-{now}-{}", std::process::id()));
        std::fs::create_dir_all(&out_dir).expect("create out dir");
        let mut step = minimal_step("s1");
        step.write_to = Some("out/result.txt".to_string());
        let completed_outputs = HashMap::from([("s1".to_string(), "abc".to_string())]);
        let disposition =
    ...
    #[test]
    fn resume_disposition_for_step_skips_when_fingerprint_matches() {
        let now = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .expect("time")
            .as_nanos();
        let out_dir =
            std::env::temp_dir().join(format!("swarm-resume-match-{now}-{}", std::process::id()));
        std::fs::create_dir_all(out_dir.join("out")).expect("create out dir");
        std::fs::write(out_dir.join("out/result.txt"), "hello\n").expect("write output");
        let mut step = minimal_step("s1");
        step.write_to = Some("out/result.txt".to_string());
        let expected = model_output_fingerprint("hello\n");
        let completed_outputs = HashMap::from([("s1".to_string(), expected)]);
        let disposition =
            resume_disposition_for_step(&step, &out_dir, &completed_outputs).expect("disposition");
        assert_eq!(
            disposition,
