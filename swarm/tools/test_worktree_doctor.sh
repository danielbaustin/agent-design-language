...
DOCTOR_SRC="$ROOT_DIR/swarm/tools/worktree_doctor.sh"
PRUNE_SRC="$ROOT_DIR/swarm/tools/worktree_prune.sh"
...
mkdir -p "$repo/swarm/tools" "$managed_root" "$codex_root"
cp "$DOCTOR_SRC" "$repo/swarm/tools/worktree_doctor.sh"
cp "$PRUNE_SRC" "$repo/swarm/tools/worktree_prune.sh"
chmod +x "$repo/swarm/tools/worktree_doctor.sh" "$repo/swarm/tools/worktree_prune.sh"
...
(
  ...
  out="$("$BASH_BIN" swarm/tools/worktree_doctor.sh --repo "$repo" --managed-root "$managed_root" --codex-root "$codex_root" --format tsv)"
  ...
  dry="$("$BASH_BIN" swarm/tools/worktree_prune.sh --repo "$repo" --managed-root "$managed_root" --codex-root "$codex_root")"
